...
export class RadioFilterBoxWidthSearch extends Component {
    constructor(props) {
        super(props);
        this.state = {
            show: false,
    ...
    cheaged = (e) => {
        ...
        const values = [];
        current.querySelectorAll(`input[type="radio"]`).forEach(item => {
                ...
                <div className="label-warp">
                    ...
                    <FormGroup className={this.state.show ? 'slideDown' : 'slideUp'}>
                        ...
                        {this.props.elementConfig.options.map(item => (
                            ...
                            <Label 
                            className={(!this.state.filterValue || item.label.toLowerCase().includes(this.state.filterValue.toLowerCase())) ? '' : 'd-none'} 
                            key={item.value}>
                                <Input type="radio" onChange={this.cheaged}
                                    checked={item.value === this.props.value}
                                value={item.value} name={this.props.name} /> {item.label}
